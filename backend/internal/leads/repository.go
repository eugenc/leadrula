...
var builtinFields = map[string]bool{
	...
	"address": true, "city": true, "state": true, "zip": true, "country": true, "address_place_id": true, "source": true,
	"external_id": true,
}

const leadCols = `id, public_id, owner_account_id, publisher_id, contract_id,
	first_name, last_name, phone, email, address, city, state, zip, country, address_place_id, source, external_id,
	cost, revenue,
...
func scanLead(row pgx.Row) (*Lead, error) {
	...
	err := row.Scan(&l.ID, &l.PublicID, &l.OwnerAccountID, &l.PublisherID, &l.ContractID,
		&l.FirstName, &l.LastName, &l.Phone, &l.Email, &l.Address, &l.City, &l.State, &l.Zip, &l.Country, &l.AddressPlaceID, &l.Source, &l.ExternalID,
		&l.Cost, &l.Revenue,
