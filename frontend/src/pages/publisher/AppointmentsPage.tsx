...
export function PublisherAppointmentsPage() {
  ...
  function pickContract(id: number) {
  ...
  return (
    <>
      ...
      <PageBody>
        {tab === "booked" ? (
          <BookedAppointmentsTable items={booked} isLoading={loadingBooked} showBuyer />
        ) : loadingContracts ? (
                ...
                <div key={b.buyer_id}>
                  ...
                  {b.contracts.map((c) => (
                    <button
                      key={c.contract_id}
                      type="button"
                      onClick={() => pickContract(c.contract_id)}
                      className={cn(
