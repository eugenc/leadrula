...
export function ApiKeysPage() {
  ...
  return (
    <div>
      <PageHeader title="API Keys" subtitle="Keys for the public lead intake API." />
      {secret && (
        <Card className="mb-4 border-pd-green/40 bg-pd-green/5 p-4">
          <div className="mb-1 text-sm font-semibold text-pd-green">Copy your key now — it won't be shown again.</div>
          <div className="flex items-center gap-2">
            <code className="flex-1 break-all rounded bg-white px-3 py-2 font-mono text-xs">{secret}</code>
            <Button
              size="icon"
              variant="outline"
              onClick={() => {
          ...
          <TBody>
            {(keys ?? []).map((k) => (
              <TR key={k.id}>
                ...
                <TD>
                  <Badge variant={k.revoked_at ? "muted" : "green"}>{k.revoked_at ? "revoked" : "active"}</Badge>
                </TD>
                <TD>
                  {!k.revoked_at && (
