...
export function PublisherBookAppointmentSheet({
  open,
  onClose,
  onBooked,
}: {
  ...
  const { data: calendars = [], isLoading: loadingCalendars } = usePublisherCalendars();
  ...
  const { data: contractMarkers = [] } = useCalendarMarkers(selectedContractId, monthStart, monthEnd);
  ...
  const { data: contractDaySlots, isLoading: loadingContractSlots, isError: contractSlotsError, error: contractSlotsErr } =
    useFreeSlots(selectedContractId, selectedDate);
  const { data: calendarDaySlots, isLoading: loadingCalendarSlots, isError: calendarSlotsError, error: calendarSlotsErr } =
    useCalendarAppointmentFreeSlots(selectedCalendarId, selectedDate, "publisher");
                ...
                <div className="flex gap-4">
                  <div className="min-w-0 flex-1">
                    ...
                    {selectedContract?.location?.trim() && (
                      <p className="mt-2 text-xs text-gray-400">Location: {selectedContract.location.trim()}</p>
                    )}
                  </div>
